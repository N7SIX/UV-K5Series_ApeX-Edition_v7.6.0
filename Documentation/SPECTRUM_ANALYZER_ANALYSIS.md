...
## Overview
The spectrum analyzer (App/app/spectrum.c) visualizes RF signals using the BK4819 RSSI readings. Features include waterfall, peak hold, auto-trigger, listening mode, and user controls.
...
### Key helpers
- `GetStepsCount()`, `GetDisplayWidth()`: calculate number of steps
...
1. **Horizontal scaling mismatch** (CRITICAL → FIXED)
   - **Issue:** Spectrum trace and waterfall used different horizontal scaling formulas
   ...
   - **Verification:** Tested with all stepsCount values
...
6. **Listening noise floor** (LOW)
   - **Issue:** Synthetic floor could mask weak signals post-listen
...
**Remaining low-risk items** are documented and have acceptable mitigations. The module now functions reliably across standard use cases.
