...
# Re-Adapted For UV-K5 EGZUMER/N7SIX custom software By JOC2
#
...
#3.5.2: 
#       modify the upload function for the user about all the n7six setting
...
DRIVER_VERSION = "Quansheng UV-K5/K6/5R driver ver: 2025/11/08 (c) EGZUMER + N7SIX v7.6.10"
FIRMWARE_VERSION_UPDATE = "https://github.com/N7SIX/UV-K5Series_ApeX-Edition_v7.6.0/releases"

CHIRP_DRIVER_VERSION_UPDATE = "https://github.com/N7SIX/UV-K5Series_ApeX-Edition_v7.6.0"
...
N7SIX_START =0x1FF2 # calibration N7SIX memory start address
...
def do_upload(radio):
    ...
    if radio.upload_calibration:
        status.max = N7SIX_START-CAL_START
        start_addr = CAL_START
        stop_addr = N7SIX_START
    ...
    while mstep < 2: # stop when 2
        ...
        if mstep == 1:   # if the first write mem done , pass to n7six value
            status.max = MEM_SIZE-N7SIX_START
            start_addr = N7SIX_START
            stop_addr = MEM_SIZE
...
# add menu firmware with version and option display if version 3.0 and up
        ...
        if self.FIRMWARE_VERSION == "":
            ...
            if self.FIRMWARE_VERSION >= Compair1 :
                if _mem.BUILD_OPTIONS.ENABLE_FEAT_N7SIX_GAME and not _mem.BUILD_OPTIONS.ENABLE_BANDSCOPE :   
                ...
                elif _mem.BUILD_OPTIONS.ENABLE_FMRADIO and _mem.BUILD_OPTIONS.ENABLE_BANDSCOPE and _mem.BUILD_OPTIONS.ENABLE_FEAT_N7SIX_GAME :         
                ...
                elif _mem.BUILD_OPTIONS.ENABLE_FEAT_N7SIX_RESCUE_OPS :   
...
#       message box to the web page link (chirp)
        ...
        append_label(roinfo,
                     "=" * 6 + " PDF File for Firmware N7SIX " + "=" * 300, "=" * 300)
...
#       message box to the web page link (PDF)
        ...
        def validate_Go_Web_Page2(value):
            if value :
                msg = "Go to the web Page of the full explanation for Firmware N7SIX \n" \
        ...
        rs = RadioSetting("Update_Pdf_0","Go to the web Page of the full explanation for Firmware N7SIX select this Box ->", val)
...
#       message box to the web page link (PDF)
        ...
        def validate_Go_Web_Page3(value):
            if value :
                msg = "Go to the web Page of the QUICK KEY For Firmware N7SIX \n" \
                      + PDF_FILE_QUICK_KEY
                ret = wx.MessageBox(msg, "Warning", wx.OK | wx.CANCEL |
                                    wx.CANCEL_DEFAULT | wx.ICON_WARNING)
                if ret == wx.OK :
        ...
        rs = RadioSetting("Update_Pdf_3","Go to the web Page of the QUICK KEY For Firmware N7SIX select this Box ->", val)
...
#       message box to the web page link (youtube)
        append_label(roinfo,
                     "=" * 6 + " YOUTUBE CHANNEL of N7SIX " + "=" * 300, "=" * 300)

        val = RadioSettingValueBoolean(False)
        def validate_Go_Web_Page3(value):
            if value :
                msg = "Go to the youtube channel of N7SIX \n" \
                      + CHAINE_N7SIX
                ret = wx.MessageBox(msg, "Warning", wx.OK | wx.CANCEL |
                                    wx.CANCEL_DEFAULT | wx.ICON_WARNING)
                if ret == wx.OK :
                    webbrowser.open(CHAINE_N7SIX)
        ...
        val.set_validate_callback(validate_Go_Web_Page3)
        rs = RadioSetting("CHAINE_YOUTUBE","Go to the youtube channel of N7SIX select this Box ->", val)
        rs.set_doc('For video explain firmware N7SIX')
        roinfo.append(rs)
...
#       message box to the web page link (youtube allemand)
        append_label(roinfo,
                     "=" * 6 + " SOME YOUTUBE LINK INFO IN SOME LANGUAGE FOR N7SIX " + "=" * 300, "=" * 300)
