...
//  uint8_t Mic;
    ...
    // Validate battery calibration - if invalid, generate safe defaults
    // Expected: [0]<[1]<[2]<[3]<[4], all in typical range 1500-3500
    if (gBatteryCalibration[0] >= 5000 || gBatteryCalibration[0] == 0 ||
        gBatteryCalibration[3] >= 5000 || gBatteryCalibration[3] == 0 ||
        gBatteryCalibration[3] <= gBatteryCalibration[0])
    {
        // Generate sensible defaults: assume 2.5V reference
        // At 2.5V ref: 5.20V = 2080 ADC, 7.60V = 3040 ADC
        gBatteryCalibration[0] = 2080;  // 5.20V empty
        gBatteryCalibration[3] = 3040;  // 7.60V full
    ...
    gBatteryCalibration[5] = 2300;  // Timestamp/reserved
