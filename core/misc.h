...
// Definition for ChannelAttributes_t (inferred from usage in settings.c and related files)
typedef union {
    struct {
        uint8_t band : 3;
        uint8_t compander : 1;
...
// Extern declaration for global ChannelAttributes array (after FREQ_CHANNEL_LAST is defined)
...
extern uint16_t            gLowBatteryPopupCountdown;   /* 500ms ticks until escalation popup, 0 = idle */
extern uint8_t             gLowBatteryEscalationLevel;  /* 0=idle, 1=popup shown, 2=critical tone */
