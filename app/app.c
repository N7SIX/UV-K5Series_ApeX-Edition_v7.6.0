...
void APP_TimeSlice10ms(void)
{
    ...
    if (UART_IsCommandAvailable()) {
        // Save current interrupt state before critical section
        uint32_t irq_state = __get_PRIMASK();
        __disable_irq();
        
        // Handle UART command in protected context
        UART_HandleCommand();
        
        // Restore previous interrupt state (don't unconditionally enable)
        if (!irq_state) {
            __enable_irq();
        }
    }
...
static void ProcessKey(KEY_Code_t Key, bool bKeyPressed, bool bKeyHeld)
{
    ...
    bool lowBatPopup = gLowBattery && !gLowBatteryConfirmed && gLowBatteryPopupCountdown == 0u && gScreenToDisplay == DISPLAY_MAIN;
