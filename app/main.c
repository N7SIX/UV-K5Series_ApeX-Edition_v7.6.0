...
static void MAIN_Key_DIGITS(KEY_Code_t Key, bool bKeyPressed, bool bKeyHeld)
{
    ...
    if (!gWasFKeyPressed) { // F-key wasn't pressed
        ...
        if (gScanStateDir != SCAN_OFF){
            ...
            switch(Key) {
                case KEY_0:
                    gEeprom.SCAN_LIST_DEFAULT = MR_CHANNEL_LAST + 1;
            ...
            */
            ...
            if (value <= MR_CHANNEL_LAST)
            {
                ...
                if (!RADIO_CheckValidChannel(value, false, 0))
                {
        ...
        if (bKeyPressed) { // long press MENU key
            ...
            if(gScanStateDir != SCAN_OFF)
            {
                if(FUNCTION_IsRx() || gScanPauseDelayIn_10ms > 9)
                {
                    ...
                    gMR_ChannelExclude[lastFoundFrqOrChan] = true;

                    SETTINGS_SaveChannel(lastFoundFrqOrChan, gEeprom.TX_VFO, gTxVfo, 1);
                    UI_MAIN_NotifyScanProgressDataChanged();
