...
void BATTERY_TimeSlice500ms(void)
{
    ...
    #define LOW_BATT_COUNTDOWN_TICKS 30u   // 15 s (30 × 500 ms) before first escalation popup
    ...
    if (gBatteryVoltageAverage <= LOW_BATTERY_THRESHOLD)
    {
        ...
        // Start / continue the escalation countdown when battery is critically low
        if (!gLowBatteryConfirmed)
        {
            if (gLowBatteryPopupCountdown == 0u)
            {
                // fresh low-battery event: arm the 15 s count before first popup
                gLowBatteryPopupCountdown = LOW_BATT_COUNTDOWN_TICKS;
                gLowBatteryEscalationLevel = 0;
            }
            else if (gLowBatteryPopupCountdown > 0u)
            {
                gLowBatteryPopupCountdown--;
            }
        }
    }
    else if (gBatteryVoltageAverage > (LOW_BATTERY_THRESHOLD + LOW_BATTERY_HYSTERESIS))
    {
        ...
        // Battery recovered: reset all escalation state so the next
        // low event starts a fresh countdown cycle.
        gLowBatteryConfirmed      = false;
        gLowBatteryPopupCountdown = 0u;
        gLowBatteryEscalationLevel = 0;
    }
    // else: maintain previous state while in hysteresis zone (50mV band)

    // Escalation: countdown expired and battery still low → show popup + play tone
    if (gLowBattery && !gLowBatteryConfirmed && gLowBatteryPopupCountdown == 0u)
    {
        AUDIO_PlayBeep(BEEP_880HZ_300MS_LOW_BATTERY_CRITICAL);
        gLowBatteryEscalationLevel = 1;
        // Popup will be drawn by the key-loop in app/app.c while this flag is set.
        // Countdown stays 0 until the user dismisses the popup.
    }
