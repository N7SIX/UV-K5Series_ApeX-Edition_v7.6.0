...
/* ---------------------------------------------------------------------
 * Approved-index lookups (K1 v7.6.10D lineage).
 *
 * PMR446 / FRS regulatory compliance: certain CTCSS tones and DCS codes
 * are not homologated for use on those services.  The ExtraIdx arrays
 * list the option indices that are EXCLUDED.  The approved-index helpers
 * return a dense index (skipping excluded entries) when Option is homologated,
 * or 0xFF when it is not.
 * --------------------------------------------------------------------- */

/* CTCSS tones that are NOT PMR446/FRS-homologated (option index in CTCSS_Options). */
const uint8_t CTCSS_ExtraIdx[12] = {
     1, 26, 28, 30, 32, 34, 36, 38, 39, 41,
    45, 49
};

/* DCS codes that are NOT PMR446/FRS-homologated (option index in DCS_Options). */
const uint8_t DCS_ExtraIdx[21] = {
     5,  9, 19, 25, 34, 36, 41, 43, 44, 48,
    50, 54, 56, 60, 71, 72, 73, 74, 75, 82,
    83
};

/* Return the dense approved index for Option within Options[], skipping the
 * entries listed in ExtraIdx[].  Returns 0xFF if Option itself is in ExtraIdx
 * (i.e. not homologated). */
static uint8_t DCS_GetApprovedIndex(uint8_t Option,
                                     unsigned int options_size,
                                     unsigned int extraidx_size,
                                     const uint8_t *extraidx)
{
    unsigned int i;
    unsigned int extra_pos = 0u;
    uint8_t      approved_index = 0u;

    if (Option >= options_size)
        return 0xFFu;

    for (i = 0u; i < options_size; i++) {
...
{
    return DCS_GetApprovedIndex(Option,
                                ARRAY_SIZE(CTCSS_Options),
                                ARRAY_SIZE(CTCSS_ExtraIdx),
                                CTCSS_ExtraIdx);
}
...
{
    return DCS_GetApprovedIndex(Option,
                                ARRAY_SIZE(DCS_Options),
                                ARRAY_SIZE(DCS_ExtraIdx),
                                DCS_ExtraIdx);
}
