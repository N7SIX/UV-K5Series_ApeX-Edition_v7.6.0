...
#define SCAN_PROGRESS_MR_CHANNEL_BYTES ((MR_CHANNEL_LAST + 7u) / 8u)
...
static void ScanProgress_RebuildMemoryMap(uint8_t scan_list)
{
    ...
    for (uint16_t ch = MR_CHANNEL_FIRST; IS_MR_CHANNEL(ch); ch++) {
        const ChannelAttributes_t *att = &gMR_ChannelAttributes[ch];
...
#define SCOPE_SAMPLES        43   // number of columns (43 Ã— 3px = 128px wide)
...
void UI_DisplayAudioScope(void)
{
    ...
    // FM RX audio is frequency-encoded â€” no register gives the instantaneous waveform.
