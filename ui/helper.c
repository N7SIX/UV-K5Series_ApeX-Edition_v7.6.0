...
void UI_FillRectangleBuffer(uint8_t (*buffer)[128], int16_t x1, int16_t y1, int16_t x2, int16_t y2, bool black)
{
    for (int16_t y = y1; y <= y2; y++) {
        for (int16_t x = x1; x <= x2; x++) {
            UI_DrawPixelBuffer(buffer, x, y, black);
        }
    }
}

void UI_Draw5x5Char(char c, uint8_t x, uint8_t y, bool black)
{
    if (c < 0x20 || c > 0x7F) return;
    uint8_t index = c - 0x20;
    for (int i = 0; i < 3; i++) {
        uint8_t pixels = gFont3x5[index][i];
        for (int j = 0; j < 5; j++) {
            if (pixels & (1 << j)) {
                UI_DrawPixelBuffer(gFrameBuffer, x + i, y + j, black);
            }
        }
    }
}

void UI_Draw5x5String(const char *pString, uint8_t x, uint8_t y, bool black)
{
    while (*pString) {
        UI_Draw5x5Char(*pString++, x, y, black);
